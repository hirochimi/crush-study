...
var infoCmd = &cobra.Command{
	Use:   "info",
	Short: "Show Crush configuration and environment information",
	Long:  `Show Crush configuration and environment information including models, providers, LSP, MCP, hooks, permissions, and options.`,
...
func buildInfo(cfg *config.ConfigStore) string {
	...
	writeLSPConfigured(&b, cfg)
	writeMCPConfigured(&b, cfg)
	writeSkills(&b, cfg)
	writeHooks(&b, cfg)
...
func writeLSPConfigured(b *strings.Builder, cfg *config.ConfigStore) {
	c := cfg.Config()
	if len(c.LSP) == 0 {
		return
	}
...
func writeMCPConfigured(b *strings.Builder, cfg *config.ConfigStore) {
	c := cfg.Config()
	if len(c.MCP) == 0 {
		return
	}
...
func writeSkills(b *strings.Builder, cfg *config.ConfigStore) {
	...
	disabled := []string{}
	if c.Options != nil {
		disabled = c.Options.DisabledSkills
	}

	// We can only show disabled skills from config since we don't have runtime skill tracker
	if len(disabled) == 0 {
		return
	}
